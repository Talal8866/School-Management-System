...
// ===== Basic Types =====
const schoolName: string = "Alzihrawi"
...
// ===== Types , Tuples =====
type StudentRecord = [string, number, number, number]; // [name, age, grade, studentId]
type GradeRecord = [string, string, number]; // [studentName, subject, grade]
type TeacherRecord = [string, number, string, number]; // [name, age, subject, teacherId]
...
interface StudentReport {
    average: number,
    studentName: string,
    grades: Array<[string, number]> // [subject, grade]
}
...
function formatGrade(grade: number): string {
    ...
    if (grade >= 60) return `${grade} Passed`
    return `${grade} Failed`
}
...
class SchoolSystem {
    ...
    private nextStudentId: number = 1000
    private nextTeacherId: number = 2000

    // ===== Student Management =====
    addStudent(name: string, age: number, grade: Grade): Student {
    ...
    // ===== Teacher Management =====
    addTeacher(name: string, age: number, subject: Subject): Teacher {
    ...
    // ===== Grade Management =====
    addGrade(studentId: number, subject: Subject, grade: number): boolean {
    ...
    // ===== Reports =====
    getStudentReport(studentid: number): StudentReport | undefined {
        const student = this.Students.find(param => param.id === studentid);
        if (!student) {
        ...
        const grades: Array<[string, number]> = studentGrades.map(param => [param.subject, param.grade]);
    ...
    // ===== Display Functions =====
    getStudentRecords(): StudentRecord[] {
    ...
    getTeacherRecords(): TeacherRecord[] {
    ...
    searchStudents(criteria: {
        ...
        return this.Students.filter(student => {
            if (criteria.name && !student.name.includes(criteria.name)) return false;
            if (criteria.grade && student.grade !== criteria.grade) return false;
            if (criteria.minAge && student.age < criteria.minAge) return false;
            if (criteria.maxAge && student.age > criteria.maxAge) return false;
            return true;
        });
    }

    // ===== Advanced Statistics =====
    getGradeDistribution(): Array<[Grade, number]> {
        const distribution: Array<[Grade, number]> = [];
    ...
    getSubjectStatistics(): Array<[Subject, number, number]> {
        const stats: Array<[Subject, number, number]> = [];
...
// ===== Output Functions =====
function printHeader(title: string, emoji: string = "🎯"): void {
    const width = 70;
...
function printTable(headers: string[], rows: string[][], title?: string): void {
    ...
    const headerRow = headers.map((header, index) =>
        ` \x1b[1m${padEnd(header, colWidths[index])}\x1b[0m `
    ).join("\x1b[37m│\x1b[0m");
    ...
    // Rows
    rows.forEach(row => {
        const rowString = row.map((cell, index) =>
            ` ${padEnd(String(cell), colWidths[index])} `
        ).join("\x1b[37m│\x1b[0m");
...
function printStatBox(title: string, value: string | number, icon: string, color: string = "36"): void {
...
function printGradeChart(grades: Array<[string, number]>): void {
